...
* Single- or multi-user deployments via variables.
* Deploys the showroom chart https://github.com/rhpds/showroom-deployer/charts/showroom-single-pod
* Uses Ansible modules (no local helm CLI required).
* Supports multiple deployment options
** Just lab content
** Lab content and a Wetty terminal that can be configured to automatically ssh to a bastion
** Lab content and an OpenShift terminal pod (images available for ocp/rosa/aro/etc)
...
If newer charts are available than the default version in the workload (`^2.0.0`) you will need to specify the version to use. If you want to use another chart (careful: it needs to be compatible) you can pick that, too.
+
.Example
[source,yaml]
----
...
ocp4_workload_showroom_deployer_chart_version: "^2.0.0"
----
...
- name: Provide installed bastion data
  agnosticd.core.agnosticd_user_info:
    data:
      bastion_ssh_command: "ssh {{ ansible_user }}@bastion.{{ guid }}.{{ azure_root_dns_zone }}"
      bastion_public_hostname: "bastion.{{ guid }}.{{ cluster_dns_zone }}"
      bastion_ssh_user_name: "{{ ansible_user }}"
...
Multi-user deployment is controlled via variables:
+
.Example
[source,yaml]
----
# Explicit per-user map (user -> credentials used by Wetty if enabled)
ocp4_workload_showroom_users:
  user1: {}
  user2: {}

# Or auto-generate N users (names user1..userN)
num_users: 2
----
